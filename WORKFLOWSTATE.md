...
- **New launcher icon** ("try another one"): replaced the 6-petal mark with a whisper-paper hourglass ("sifting time") on the terracotta `#D97757` bg: NEW `drawable/ic_launcher_foreground.xml` (vector, adaptive safe zone; old `drawable-nodpi/ic_launcher_foreground.png` deleted — adaptive icon referenced @drawable/ic_launcher_foreground, which now resolves to the XML), legacy `mipmap-{mdpi,hdpi,xhdpi,xxhdpi,xxxhdpi}/ic_launcher.png` regenerated programmatically (GDI+), `ic_stat_sift.xml` (notification) = white hourglass silhouette.
